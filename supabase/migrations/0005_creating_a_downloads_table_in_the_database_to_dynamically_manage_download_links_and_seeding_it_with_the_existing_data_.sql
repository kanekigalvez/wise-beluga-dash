...
CREATE TABLE public.downloads (
  ...
  category TEXT NOT NULL CHECK (category IN ('diagzone', 'xpro', 'xdiag')),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Add comments for clarity
COMMENT ON TABLE public.downloads IS 'Stores download links for the downloads page.';
COMMENT ON COLUMN public.downloads.category IS 'Category for grouping downloads, e.g., diagzone, xpro, xdiag.';
...
CREATE POLICY "Public can read downloads" ON public.downloads
  FOR SELECT USING (true);

-- 2. Allow authenticated users to manage downloads.
-- NOTE: For a production app with open registration, this should be restricted to a specific admin role.
